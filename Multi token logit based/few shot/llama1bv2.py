...
def compute_logprob_batch_llama(prompts, label_text, tokenizer, model, device, max_len):
...
def main():
    ...
    tracker = EmissionsTracker(
        project_name="llama-teacher-forcing",
    ...
    for i in tqdm(range(0, len(rows), BATCH_SIZE)):
        ...
        s_scores = compute_logprob_batch_llama(
            prompts, " sarcastic", tokenizer, model, device, args.max_input_length
        )

        n_scores = compute_logprob_batch_llama(
