...
# ---------------- LOGIT FUNCTION ----------------
def compute_logprob_batch_qwen(prompts, label_text, tokenizer, model, device, max_len):
    ...
    # ✅ SAFE prompt length (no padding confusion)
    prompt_lengths = [
...
# ---------------- LOGIT INSPECTION ----------------
def inspect_sample(text, tokenizer, model, device, max_len=512):
    ...
    for label in labels:
        ...
        for t in range(prompt_len, attention_mask.sum().item()):
...
def main(args):
    ...
    tracker = EmissionsTracker(
        project_name="Qwen-LoRA-Logit-Eval",
    ...
    for i in tqdm(range(0, len(df), BATCH_SIZE)):
        ...
        s_scores = compute_logprob_batch_qwen(
            prompts, " sarcastic", tokenizer, model, device, MAX_INPUT_LENGTH
        )

        n_scores = compute_logprob_batch_qwen(
