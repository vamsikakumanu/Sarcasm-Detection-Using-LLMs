...
def map_label_from_text(gen: str) -> str:
    ...
    s = re.sub(r"^[\"'`]+|[\"'`]+$", "", s)
    if re.search(r'\bnot[-\s]*sarcastic\b', s):
        return "not sarcastic"
    if re.search(r'\bsarcasm\b', s) or re.search(r'\bsarcastic\b', s):
        return "sarcastic"
    if re.search(r'classification[:\s]*not', s):
        return "not sarcastic"
    ...
    if toks[0] in ("not","no"):
        return "not sarcastic"
    if toks[0] in ("sarcastic","yes","y","yeah","yep","classification:","the","this"):
        return "sarcastic"
    return "not sarcastic"
...
def main():
    parser = argparse.ArgumentParser(description="Llama-3.2 zero-shot CSV evaluator (batched)")
...
# ===============================================================
    ...
    try:
        for start in tqdm(range(0, total, batch_size), desc="Batches"):
            ...
            for row in batch_rows:
                ...
                messages = [
    {"role": "system", "content": "classify the user's text either 'sarcastic' or 'not sarcastic' answer in one word"},
    {"role": "user", "content": f"User text to classify: {text}"} ]
            ...
            for i, out_ids in enumerate(out):
                ...
                prompt = prompts[i]
                gen_text = extract_generated_text(full, prompt)
