...
MAX_NEW_TOKENS = 6   # reduced for speed
...
def main(args):
    ...
    tracker = EmissionsTracker(
        project_name="SmolLM1.7B-LoRA-Inference",
    ...
    for start in tqdm(range(0, len(df), BATCH_SIZE), desc="Evaluating SmolLM-LoRA"):
        ...
        for i, raw_answer in enumerate(decoded_batch):
            ...
            gold = "sarcastic" if str(golds[i]).lower() in ["1","sarcastic","true","yes"] else "not_sarcastic"

            gold_labels.append(gold)
            pred_labels.append(pred)
            raw_generations.append(raw_answer.strip())

            # 🔍 Inspect first 20 predictions live
            global_idx = start + i
            if global_idx < 100:
                print("=" * 80)
                print(f"IDX   : {global_idx}")
                print(f"TEXT  : {texts[i][:250]}")
                print(f"GEN   : {raw_answer.strip()}")
                print(f"PRED  : {pred}")
                print(f"GOLD  : {gold}")
    ...
    precision, recall, f1, _ = precision_recall_fscore_support(
        filtered_gold, filtered_pred,
        average="binary", pos_label="sarcastic", zero_division=0
    )
